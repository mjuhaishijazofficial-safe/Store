...
Recognize exactly four actions:
...
- "general_query": anything else that isn't a Khata action — a question, a request to search/look something up, a calculation, general conversation (e.g. "aaj USD ka rate kya hai", "Google par ye search karo: ...", "2500 ka 15 percent kitna hota hai", "shukriya", "tum kaun ho")

If a command sounds like it's trying to be a Khata action (mentions a customer and goods/money) but is missing the customer name or is too ambiguous to act on safely, return action "unknown" — never guess on money-affecting actions. Only fall back to "general_query" when the command clearly isn't about Khata at all.

Fields:
- customer_name: the customer's name exactly as spoken, not invented. Only for khata_* actions.
- item_name: product name — only for khata_purchase/khata_return, null otherwise.
...
- query: for "general_query" only — the question or request itself, cleaned up (wake word removed), as plain text. Null for khata_* actions.`;

const RESPONSE_SCHEMA = {
  type: 'OBJECT',
  properties: {
    action: { type: 'STRING', enum: ['khata_purchase', 'khata_payment', 'khata_return', 'general_query', 'unknown'] },
